...
@php $size = request('size', '80'); @endphp
<style>
...
<div class="print-options" id="printOptions">
    ...
    <select id="receiptSize" onchange="changeReceiptSize(this.value)" style="width:100%; padding:5px; margin-bottom:8px; border-radius:4px; border:1px solid #ddd;">
        <option value="58" {{ $size == '58' ? 'selected' : '' }}>58mm (Small)</option>
        <option value="80" {{ $size == '80' ? 'selected' : '' }}>80mm (Standard)</option>
    </select>
    <label>
        <input type="checkbox" id="showLogo" {{ $order->store?->logo ? '' : 'disabled' }}>
        Show Business Logo
    </label>
    <label>
...
<div class="text-center">
    @if($order->store?->logo)
    <img src="{{ asset('storage/' . $order->store->logo) }}" class="store-logo" id="storeLogo" style="display:none;" alt="Logo" onerror="this.style.display='none'">
    @endif
    <div class="bold store-name" style="font-size:16px">{{ $order->store?->name ?? 'My Store' }}</div>
    @if($order->store?->address)<div>{{ $order->store->address }}</div>@endif
    @if($order->store?->phone)<div>Tel: {{ $order->store->phone }}</div>@endif
    @if($order->store?->email)<div>{{ $order->store->email }}</div>@endif
    @if($order->store?->receipt_header)<div style="margin-top:4px;font-style:italic">{{ $order->store->receipt_header }}</div>@endif
</div>
<hr>
...
@if($order->customer)<div>Customer: {{ $order->customer->name }}</div>@endif
<hr>
@foreach($order->items as $item)
<div>{{ $item->product_name }}</div>
<div class="item-row"><span>{{ $item->quantity }} x Rs.{{ number_format($item->unit_price,2) }}</span><span>Rs.{{ number_format($item->total,2) }}</span></div>
@endforeach
<hr>
<div class="item-row"><span>Subtotal</span><span>Rs.{{ number_format($order->subtotal,2) }}</span></div>
@if($order->discount_amount > 0)<div class="item-row"><span>Discount</span><span>-Rs.{{ number_format($order->discount_amount,2) }}</span></div>@endif
@if($order->tax_amount > 0)<div class="item-row"><span>Tax</span><span>Rs.{{ number_format($order->tax_amount,2) }}</span></div>@endif
<hr>
<div class="item-row grand-total"><span>TOTAL</span><span>Rs.{{ number_format($order->total_amount,2) }}</span></div>
@foreach($order->payments as $p)<div class="item-row"><span>{{ ucfirst(str_replace('_',' ',$p->method)) }}</span><span>Rs.{{ number_format($p->amount,2) }}</span></div>@endforeach
@if($order->change_amount > 0)<div class="item-row"><span>Change</span><span>Rs.{{ number_format($order->change_amount,2) }}</span></div>@endif
<hr>
<div class="text-center">
